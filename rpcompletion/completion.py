...
from brs_utils import insert_and_or_replace_in_sorted_list, Item, Cache
...
def rp_completion(
    ...
    ## READ
    __rp2paths_compounds_in_cache(infile=rp2paths_compounds, cache=cache, logger=logger)
    pathways, transfos = __read_pathways(infile=rp2paths_pathways, logger=logger)
    ec_numbers = __read_rp2_metnet(infile=rp2_metnet, logger=logger)
    sink_molecules = __read_sink(infile=sink, logger=logger)
    if cofile is not None:
        # Read cofactors
        cofactors = __parse_cofactors(cofile=cofile, logger=logger)
    else:
        cofactors = []
    ...
    pathway_combinatorics = __build_pathway_combinatorics(
        full_transfos, pathways, cache=cache, logger=logger
    )
...
def __complete_transformations(
    ...
    for transfo_id, transfo in transfos.items():
        ...
        logger.debug(
            f'full_transfos[{transfo_id}]["left"]: {full_transfos[transfo_id]["left"]}'
        )
        full_transfos[transfo_id]["right"] = dict(transfos[transfo_id]["right"])
        logger.debug(
            f'full_transfos[{transfo_id}]["right"]: {full_transfos[transfo_id]["right"]}'
        )
        full_transfos[transfo_id]["complement"] = {}
        logger.debug(
            f'full_transfos[{transfo_id}]["complement"]: {full_transfos[transfo_id]["complement"]}'
        )
            ...
            try:
                ...
                if not forward:
                    for rxn_id, _transfo in full_transfo.items():
                        ...
                        (
                            _transfo["added_cmpds"]["right"],
                            _transfo["added_cmpds"]["left"],
                        ) = (
                            _transfo["added_cmpds"]["left"],
                            _transfo["added_cmpds"]["right"],
                        )
                full_transfos[transfo_id]["complement"][rule_id] = deepcopy(
                    full_transfo
                )
                logger.debug(
...
def __build_smiles(side: Dict, logger: Logger = getLogger(__name__)) -> str:
...
def __build_reader(
    path: str, delimiter: str = ",", logger: Logger = getLogger(__name__)
) -> "csv_reader":
    ...
    if isinstance(path, bytes):
        reader = csv_reader(StringIO(path.decode("utf-8")), delimiter=delimiter)
    else:
        try:
            reader = csv_reader(open(path, "r"), delimiter=delimiter)
...
def __rp2paths_compounds_in_cache(
    infile: str, cache: rrCache, logger: Logger = getLogger(__name__)
) -> None:
    ...
    try:
        reader = __build_reader(path=infile, delimiter="\t", logger=logger)
        if reader is None:
        ...
        for row in reader:
            ...
            cmpd = __get_compound_from_cache(
                spe_id=spe_id, smiles=smiles, cache=cache, logger=logger
            )
...
def __get_compound_from_cache(
    spe_id: str, smiles: str, cache: rrCache, logger: Logger = getLogger(__name__)
) -> Dict[str, str]:
    ...
    try:
        ...
        try:
            resConv = cache._convert_depiction(
                idepic=smiles, itype="smiles", otype={"inchi"}, logger=logger
            )
            inchi = resConv["inchi"]
        except NotImplementedError:
            logger.warning(
                "Could not convert the following SMILES to InChI: " + str(smiles)
            )
    try:
    ...
    # TODO: consider using the inchi writing instead of the SMILES notation to find the inchikey
        ...
        try:
            resConv = cache._convert_depiction(
                idepic=smiles, itype="smiles", otype={"inchikey"}, logger=logger
            )
            inchikey = resConv["inchikey"]
        except NotImplementedError:
            logger.warning(
                "Could not convert the following SMILES to InChI key: " + str(smiles)
            )
    try:
    ...
    return {"inchi": inchi, "inchikey": inchikey, "name": name, "formula": formula}


def __read_rp2_metnet(infile: str, logger: Logger = getLogger(__name__)) -> Dict:
...
def __read_sink(infile: str, logger: Logger = getLogger(__name__)) -> List[str]:
    ...
    if reader is None:
        logger.warning(
            f"The sink file {infile} is empty or not found. Continuing without sink molecules."
        )
        return []
    for row in reader:
...
def __read_pathways(
    infile: str, logger: Logger = getLogger(__name__)
) -> Tuple[Dict, Dict]:
        ...
        for transfo_id in transfos_lst:
            ...
            for rule_ids, tmpl_rxns in full_transfos[transfo_id]["complement"].items():
                ...
                for tmpl_rxn_ids, tmpl_rxn in tmpl_rxns.items():
                    ...
                    compounds = __add_compounds(compounds, tmpl_rxn["added_cmpds"])
...
def __build_all_pathways(
    ...
    for path_idx, transfos_lst in pathways.items():
        ...
        for sub_path_idx in range(len(sub_pathways)):
            ...
            for rxn_idx in range(nb_reactions):
                ...
                added_cmpds = transfo["complement"][rule_ids][tmpl_rxn_id][
                    "added_cmpds"
                ]
                ...
                target_id = [
                    spe_id for spe_id in rxn.get_products_ids() if "TARGET" in spe_id
                ]
                if target_id != []:
                ...
                pathway.add_species_group(
                    ...
                    [
                        spe_id
                        for value in core_species.values()
                ...
                pathway.add_species_group(
                    ...
                    [
                        spe_id
                        for value in added_cmpds.values()
                        for spe_id in value.keys()
                    ],
                )
            ...
            pathway.set_sink_species(
                list(set(pathway.get_species_ids()) & set(sink_molecules))
            )
            ...
            res_pathways[path_idx] = __keep_unique_pathways(
                res_pathways[path_idx], pathway, logger
            )
    ...
    if maxsubpaths < 1:
        pathways = sum([pathways for pathways in res_pathways.values()], [])
    else:
        pathways = sum(
    ...
    logger.info("Pathways statistics")
    logger.info("-------------------")
    ...
    return [pathway.object for pathway in pathways]
...
def __add_compounds(
    compounds: Dict, compounds_to_add: Dict, logger: Logger = getLogger(__name__)
) -> Dict:
...
def __keep_unique_pathways(
    pathways: List[Dict], pathway: rpPathway, logger: Logger = getLogger(__name__)
) -> List[Dict]:
    """
    ...
    for _pathway in pathways:
        if pathway == _pathway.object:
            pathway_found = True
            logger.debug(
                f"Equality between {_pathway.object.get_id()} and {pathway.get_id()}"
            )
    ...
    if not pathway_found:
        ...
        pathways = insert_and_or_replace_in_sorted_list(Item(pathway, score), pathways)
