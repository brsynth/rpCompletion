...
def completeReac(cache, step, rr_reac, full_reac, mono_side, rr_string, pathway_cmp):
    ...
    rr_string += add_side_species(cache, step, full_reac, rr_reac)

    ## Update the stochio
    return True, update_stochio(cache, step, full_reac, rr_string, pathway_cmp)


def add_side_species(cache, step, full_reac, rr_reac):
    rr_string = ''
...
def update_stochio(cache, step, full_reac, rr_string, pathway_cmp):
...
def addCofactors(cache, rpsbml, compartment_id='MNXC3', pathway_id='rp_pathway'):
    ...
    #for stepNum in sorted(list(rp_path)):
        if addCofactors_step(cache, rp_path[stepNum], pathway_cmp):
